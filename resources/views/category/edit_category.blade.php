            ...
            <section class="panel">
                ...
                <div class="panel-body">
                    ...
                    @foreach($edit_category as $edit_cate)
                        <form role="form" action="{{ url('/post_update_category/'.$edit_cate->category_id) }}" method="POST">
                        {{csrf_field()}}
                            <div class="form-group">
                                <label for="exampleInputEmail1">ID Category</label>
                                <input type="text" name="id_category" value="{{$edit_cate->category_id}}" class="form-control" id="exampleInputEmail1" placeholder="ID Category" readonly>
                            </div>
                            <div class="form-group">
                                <label for="exampleInputEmail1">Name Category</label>
                                <input type="text" name="name_category" value="{{$edit_cate->category_name}}" class="form-control" id="exampleInputEmail1" placeholder="Name Category">
                            </div>
                            <div class="form-group">
                                <label for="exampleInputPassword1">Description</label>
                                <textarea class="form-control" name="des_category" id="exampleInputPassword1" placeholder="Description">{{$edit_cate->category_des}}</textarea>
                            </div>
                            <div class="form-group-status">
                                ...
                                <select name="status_category" class="form-control input-lg m-bot15">
                                    <?php
                                        ...
                                        {
                                    ?>
                                    <option value="{{$edit_cate->category_status}}">Hide</option>
                                    <option value="1">Display</option>
                                    <?php
                                        }
                                        else{
                                    ?>
                                    <option value="{{$edit_cate->category_status}}">Display</option>
